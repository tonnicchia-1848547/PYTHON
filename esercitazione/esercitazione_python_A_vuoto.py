...
def check_int_str(stringa, intero):
    intero = str(abs(intero))
    c = 0
    for l in range(len(stringa)):
        if stringa[l] == '-':
            continue
        elif stringa[l] == intero:
            c +=1
        else:
            continue
    return c
...
evaluation.evaluate(sum_multipli)
...
def get_list_except_min_max(L):
    return  L.pop(L.index(min(L))), L.pop(L.index(max(L)))
...
def get_list_except_min_max_general(L):
    c = 0
    minimo, massimo = get_list_except_min_max(L)
    # se la lista è vuota oppure minimo e massimo sono uguali
    if L == []:
        return c
    elif minimo == massimo:
        for numero in L:
